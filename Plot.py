...
def humeur(df_temp):
    ...
    valeur_humeur = np.mean(df_temp["score_pondere"])
...
def evolution(df_temp):
    ...
    df_temp["SEMAINE"] = df_temp.DATE.apply(
        lambda x: x.strftime('%Y-semaine-%U'))

    df_aggr_JOUR = df_temp.sort_values("DATE").groupby(
        "DATE").agg({"score_pondere": "mean"})
    df_aggr_SEMAINE = df_temp.sort_values("SEMAINE").groupby(
        "SEMAINE").agg({"score_pondere": "mean"})
    df_aggr_MOIS = df_temp.sort_values("MOIS").groupby(
        "MOIS").agg({"score_pondere": "mean"})
...
def categorie(df_temp):
    barplot = df_temp.groupby("Topics_text").agg(
        {"post": "count", "score_pondere": "mean"}).reset_index()
    ...
    fig = px.bar(barplot,
                 ...
                 range_color=(-0.6,
                              0.6),
                 title='Top 10 des sujets les plus discutés')
...
def report(df_post, df_comment):
    ...
    df_alerte = df_post[(df_post.Sentiments_comment < 0)
                        | (df_post.score_pondere < 0)]
    ...
    pdf.cell(
        w=0,
        h=45,
        txt=f"Rapport Smart-City du {str(derniere_date)[:10]}",
        align='C',
        ln=1,
        border=1)
